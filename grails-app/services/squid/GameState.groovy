...
class GameState
{
    ...
    {
        def playerState = playerStates.find() {it.player.name == playerName}
        (Math.abs(row - playerState.row) <= game.rowsPlayerCanMove) &&
        (Math.abs(column - playerState.column) <= game.columnsPlayerCanMove) &&
